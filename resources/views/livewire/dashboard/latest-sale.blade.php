<div>
        ...
        <div class="bg-white block-main row m-0 p-0 pt-3  table-container h-100  d-none d-sm-flex">
            <div class="col-12 p-0">
                ...
                <div class="row col mb-24 ms-4">
                    <h6 class="fw-bold p-0">Recent Sales</h6>
                </div>
                <div class="half-table-container">
                    <x-data-table.table :model="$certificates" :columns="[]" :wantSearching="true" :dateFilter="true">
                        <x-slot name="head">
                            <tr>
                                <th data-filter-control-placeholder="Name" data-field="Name" style="width: 200px">
                                    Name
                                </th>
                                <th data-filter-control-placeholder="Value" data-field="Value">Value</th>
                                <th data-filter-control-placeholder="Quantity" data-field="Quantity"> Quantity</th>
                                <th data-filter-control-placeholder="Date" data-field="Date"> Date</th>
                            </tr>
                        </x-slot>
                        <x-slot name="body">
    ...
    @else
        <div class="bg-white block-main row m-0 p-0 pt-3  table-container h-50 empty-dash">
            <div class="col-12 p-0">
                <div class="row col flex-column justify-content-center align-items-center">
                    <h6 class="fw-bold mb-2 maxw-210">Recent Sales</h6>
                    <p class="mb-2 text-black-50 maxw-200">There are no recent sales
                        yet to present..</p>
                    <a class="btn button-green mx-auto" href="{{url('/sell')}}">Start selling certificates</a>
                </div>
            </div>
        </div>
    @endif
    @if($certificates->count() > 0)
        <div class="index-list-sm d-block d-sm-none p-0">
            <h4 class="fw-bold p-0 mb-4">Recent Sales</h4>
            @foreach($certificates as $certificate)
                <div class="card-el p-3 mb-3">
                        ...
                        <a  class="text-decoration-none text-black">
                            <div class="d-flex align-items-center">
                                <svg class="icon {{$certificate->certificate->project_type->image}} me-2" width="32"
                                     height="32">
                                    <use href="./img/icons.svg#{{$certificate->certificate->project_type->image}}">
                                    </use>
                                </svg>
                    ...
                    <span class="price fw-bold"> ${{number_format($certificate->price,'2')}}</span>
{{--                    <span class="statistic-price statistic-decrease d-flex  ms-1">--}}
                    ...
                    <div class="card-body">
                        <p class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <span
                                class="result fw-bolder w-75 text-end">{{$certificate->quantity}}</span>
                        </p>
                        <p class="d-flex justify-content-between align-items-center mb-2">
                            <span class="title fw-bold text-black-50 fw-bolder">Date:</span>
                            <span
                                class="result fw-bolder w-75 text-end">{{Carbon\Carbon::parse($certificate->created_at)->format('d/m/Y')}}</span>
                        </p>
